...
- Helm 3（缺失时启动脚本会自动安装）
- 本地模式不需要 HyperDX Cloud API 密钥；切换到 Cloud 模式时才需要配置密钥
...
完整演示顺序：1) 打开应用入口，页面默认开始会话回放；2) 点击 `Mark demo action`，页面会显示已记录时间，再到 HyperDX Sessions 查看当前会话；3) 在 Traces 中按 `k3s-gofr-web`、`k3s-gofr-orders` 或 `k3s-gofr-catalog` 查看浏览器 → Envoy Gateway → orders → catalog → PostgreSQL/MySQL/Redis 链路，并在 Cilium/Hubble 中查看 Pod 流量；4) 在 Prometheus 查询 `gofr:http_responses:rate5m`、`gofr:http_response_duration_seconds:p95`、`catalog_cache_operations_total` 或 `up{job="gofr-services"}`；5) 运行 `./scripts/load-test.sh`，观察 `kubectl -n gofr-demo get hpa,pods -w` 中 orders 扩容；6) 在 Kite 查看 Gateway、Deployment、Pod、HPA 和事件；7) 查看 Cilium：`kubectl -n kube-system get pods -l k8s-app=cilium`。`Pause recording` / `Resume recording` 只控制当前浏览器会话的录制器，不会删除已有回放。
...
`services/go.mod`、`services/go.sum` 和 `services/go.work` 都属于后端目录：当前仓库只有一个 Go 模块，工作区文件放在模块旁边可以让 `go test ./...`、`go vet ./...` 和 `go build ./...` 的作用域清晰一致。根目录 `Makefile` 只负责进入 `services/` 调用这些命令；基础设施、前端和脚本不放进后端模块，避免把 Kubernetes YAML 或 Node 依赖混入 Go 模块的依赖图。
...
本地 `make up` 使用 `services/Dockerfile.runtime` 把宿主机编译出的二进制打包进运行时镜像；这样启动脚本不需要在 Docker build 阶段再次访问 Go 模块下载源。

GoFr 默认 HTTP 端口是 8000、指标端口是 2121；本示例部署环境使用默认端口。GoFr 自动提供 `/.well-known/health`、`/.well-known/alive` 和 `/metrics`。

Envoy Gateway 监听器使用 80，生成的 Envoy 代理 Service 是 ClusterIP，因此本机 `8080:80` port-forward 对浏览器保持普通 HTTP 入口。Web Deployment 内的 Nginx 只监听 8080、提供 Vue 静态文件，`/api` 完全由 Gateway API 转发到 orders。

### 设计说明

- GoFr 自带 HTTP 请求链路追踪、结构化日志、Prometheus 指标和健康探针；通过 `TRACE_EXPORTER=otlp` 发到 Collector。
- `orders` 用 W3C `traceparent` 传播头调用 `catalog`，一次浏览器请求会形成浏览器 → Gateway → orders → catalog 的完整链路。
- Cilium 提供集群 CNI、网络策略、Hubble 和 eBPF 数据路径；当前 k3d 默认保留 kube-proxy，以确保 k3s 的 API、DNS 和 ClusterIP 在本机多节点环境稳定。`kubeProxyReplacement=false` 是兼容性选择，不代表 Cilium 不工作。
- Cilium 也支持 `kubeProxyReplacement=true`，但这不是对现有集群的热切换：必须重新创建无 kube-proxy 集群，并同时验证 API server 可达性、NodePort/HostPort 和 Cilium socket-LB 设置。仅把 values 中的布尔值改成 `true` 会让当前 k3d 配置的系统服务失效，所以示例默认保留 `false`；生产环境可按 Cilium 的 kube-proxy replacement 专用集群方案单独启用并重新验收。
- Envoy Gateway 是实现 Gateway API 的控制器。`GatewayClass` 的 `controllerName` 固定为 `gateway.envoyproxy.io/gatewayclass-controller`。
